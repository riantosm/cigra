...
- Kirim sinyal darurat kini hampir instan: memakai lokasi terakhir dari pelacakan latar (kalau
  umurnya ≤ 1 menit dan akurasinya ≤ 50 m), dan baru mencari GPS baru kalau lokasi itu tidak ada
  atau kurang akurat.
- Layar "Memeriksa aplikasi" setelah login: menampilkan sapaan "Selamat pagi/siang/sore/malam,
  <nama>", izin diminta berurutan (notifikasi → lokasi → lokasi sepanjang waktu) paling lambat
  5 detik setelah layar dibuka, dan layar tertahan (dicek ulang tiap 5 detik) sampai izin lokasi
  & "Izinkan sepanjang waktu" diberikan. Tombol "Lewati" dihapus.

### Perbaikan

- Dialog izin lokasi saat login pertama tidak lagi tertahan di belakang dialog izin notifikasi
  (sebelumnya baru muncul setelah loading "Memeriksa aplikasi" selesai).
- Push notification sempat muncul dua kali saat aplikasi sedang dibuka (foreground).
...
- Home Komandan: "Ringkasan Situasi" jadi satu kartu hero gradient (ganti grid statistik +
  banner darurat terpisah); ikon quick action, Aktivitas Terbaru, dan Pengumuman Terbaru kini
  memakai chip gradient (bukan tint flat), dan kartu Peta Personel dapat bingkai gradient +
  chip info mengambang. Gaya chip ikon gradient yang sama diterapkan juga ke Kirim Pengumuman,
  Kekuatan Apel, Buku Saku, Riwayat Kesehatan, Notifikasi, Home Anggota, dan hampir seluruh
...
- Disposisi Surat: detail disposisi & pencarian penerima menampilkan foto asli pengirim/anggota
  (bukan cuma inisial).
- Kekuatan Apel: daftar Hadir/Absen/Belum, pencarian personel, scan QR, dan form absen menampilkan
  foto asli anggota, bukan cuma inisial.
- Peta Personel & Lokasi Personel: marker & daftar memakai foto asli personel (jatuh ke inisial
  kalau tidak ada/gagal dimuat). Marker berdekatan otomatis digabung jadi grid foto (maks. 4,
  sisanya badge "+N"), terpisah lagi begitu di-zoom. Tap marker menyoroti & membuka carousel kartu
  profil (avatar, nama, jabatan, status hidup berjalan, tombol Lokasi/Lihat Detail) — satu kartu
  per personel kalau markernya gabungan. Kedua halaman saling terhubung lewat ikon di header, dan
  keduanya polling posisi otomatis tiap menit selagi dibuka.

### Perbaikan

- Peta Personel: marker foto personel kadang tampil kosong walau fotonya valid (balapan snapshot
  peta Android saat foto baru selesai dimuat, dan bitmap yang kadung salah tidak diminta ulang
  saat peta digeser/di-zoom) — sekarang selalu tersinkron dengan tampilan terkini.
...
- Smart Academy: menu Academy dirombak jadi modul belajar–ujian–monitoring yang menyesuaikan
  peran. Anggota bisa lihat program yang ditugaskan, buka materi, kerjakan assessment (timer di
  server, jawaban tersimpan otomatis), lihat hasil & kemampuan, dan input hasil praktik mandiri.
  Instruktur meninjau (verifikasi/tolak/tetapkan nilai) hasil praktik mandiri anggota dan melihat
  program yang ditangani. Komandan mendapat ringkasan satuan (program aktif, completion & pass
  rate, hal yang perlu perhatian) dan daftar program.
- Modul Academy lama (Beranda/Akademik/Psikologi/Jasmani/Riwayat + tryout TKD data contoh)
  digantikan Smart Academy.
