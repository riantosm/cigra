...
> [!DONE] Rute patroli + checkpoint berurut. Entry point: quick action "Patroli" di Home Komandan (semua role `komandan`). 6 layar `Patrol*`, `services/api/patrol.service.ts`. Layar `Patrol` (beranda) & `PatrolRouteDetail`. Diverifikasi via API live 2026-09-07.
...
Catatan: `checkpoints[]` diurutkan FE by `sequence_order`. Kartu rute menampilkan `name` + `code` + jumlah checkpoint; `PatrolRouteDetail` menampilkan timeline checkpoint (kode QR, radius, koordinat).
...
- Progres = `completed_checkpoints` / `total_checkpoints`. `logs[]` (bentuk belum final) dipakai best-effort untuk stempel waktu tiap checkpoint.

### 10.3 Mulai Patroli Baru

> [!DONE] `POST /patrols/sessions/start`. Diverifikasi via API live 2026-09-07: `200` `{ success:true, message, data:<sesi, bentuk = 10.2> }`. Layar `PatrolStart` (pilih rute + catatan) → sukses → `PatrolActive`.
...
> [!PARTIAL] Endpoint jalan (`GET /patrols/sessions/history` + `GET /patrols/sessions/{session}`), sudah ada di `patrol.service.ts` (`getPatrolHistoryApi`, `getPatrolSessionApi`) — **belum dipakai layar mana pun** (belum ada artboard untuk layar riwayat/detail sesi). Diverifikasi via API live 2026-09-07.
...
### 10.6 Verifikasi Checkpoint (Scan QR + Foto Bukti)

> [!TODO] **Belum ada endpoint.** Layar `PatrolScan` (kamera QR + cek izin kamera) & `PatrolPhoto` (selfie bukti + tinjau) sudah dibangun dengan navigasi berfungsi, tapi tombol **"Kirim Bukti" dinonaktifkan** + ada banner "menunggu endpoint". `PatrolScan` punya tombol `[DEV]` (`__DEV__` only) untuk simulasi scan benar → lanjut ke `PatrolPhoto`.

Butuh dari backend: endpoint untuk mencatat kunjungan checkpoint (mis. `POST /patrols/sessions/{session}/checkpoints` dengan `qr_code` + `latitude`/`longitude` + foto `file`), yang menaikkan `completed_checkpoints` dan mengisi `logs[]`. Sudah dicek 2026-09-07 — pola path umum (`/scan`, `/checkpoints`, `/checkpoints/{id}/scan`, `PATCH /sessions/{id}`, dst.) semua `404`/`405`.
